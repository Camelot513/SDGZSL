...
class DATA_LOADER(object):
    def __init__(self, opt):
        ...
        for i in range(self.seenclasses.shape[0]):
            self.tr_cls_centroid[i] = np.mean(self.train_feature[self.train_label == i].cpu().numpy(), axis=0)
    ...
    def read_matdataset(self, opt):
        ...
        # self.test_seen_label = torch.from_numpy(label[test_seen_loc]).long()
        # ours
        _train_feature = np.array(feature[trainval_loc])
        _train_label = np.array(label[trainval_loc])
        _test_seen_feature = np.array(feature[test_seen_loc])
        _test_seen_label = np.array(label[test_seen_loc])
        _test_unseen_feature = np.array(feature[test_unseen_loc])
        _test_unseen_label = np.array(label[test_unseen_loc])
        self.train_feature = torch.from_numpy(_train_feature).float().cuda()
        self.train_feature = F.normalize(self.train_feature, p=2, dim=self.train_feature.dim()-1, eps=1e-12)
        self.train_label = torch.from_numpy(_train_label).long()
        self.test_seen_feature = torch.from_numpy(_test_seen_feature).float().cuda()
        self.test_seen_feature = F.normalize(self.test_seen_feature, p=2, dim=self.test_seen_feature.dim()-1, eps=1e-12)
        ...
        self.test_unseen_feature = F.normalize(self.test_unseen_feature, p=2, dim=self.test_unseen_feature.dim()-1, eps=1e-12)
