...
def train():
    ...
    out_dir = 'out/{}/wd-{}_b-{}_g-{}_lr-{}_sd-{}_dis-{}_nS-{}_nZ-{}_bs-{}_CUB_H_modify_1018'.format(opt.dataset, opt.weight_decay,
    ...
    neta = attNetwork(opt).to(opt.gpu)
    print(model)
    ...
    optimizer_neta = torch.optim.Adam(neta.parameters(), lr=opt.lr)
    ...
    for it in range(start_step, opt.niter+1):
        ...
        pred_att1 = neta(hs1)
        pred_att2 = neta(hs2)
        loss_att = mse(pred_att1.cuda(), C.cuda()) + mse(pred_att2.cuda(), C.cuda())
        loss = loss + loss_att
        ...
        if it % opt.disp_interval == 0 and it:
            log_text = 'Iter-[{}/{}]; loss: {:.3f}; kl:{:.3f}; p_loss:{:.3f}; rec:{:.3f}; tc:{:.5f}; gamma:{:.3f};'.format(it,
        ...
        if it % opt.evl_interval == 0 and it > opt.evl_start:
            ...
            if opt.zsl:
                ...
                # if result_gzsl_soft.save_model:
                #     model_path = out_dir + '/Best_model_save_H_{:.2f}_S_{:.2f}_U_{:.2f}.pth'.format(result_gzsl_soft.best_acc,
                #                                                                                  result_gzsl_soft.best_acc_S_T,
                #                                                                                  result_gzsl_soft.best_acc_U_T)
                #     torch.save(model, model_path)
                # if result_gzsl_soft.save_model:
                #     model_path = out_dir + '/Best_ae_save_H_{:.2f}_S_{:.2f}_U_{:.2f}.pth'.format(result_gzsl_soft.best_acc,
                #                                                                                  result_gzsl_soft.best_acc_S_T,
                #                                                                                  result_gzsl_soft.best_acc_U_T)
                #     torch.save(ae, model_path)
                # 最新的网络保存
                if result_gzsl_soft.save_model:
                    files2remove = glob.glob(out_dir + '/Best_model_newGZSL_*')
                    for _i in files2remove:
                        os.remove(_i)
                    model_path = out_dir + '/Best_model_newGZSL_H_{:.2f}_S_{:.2f}_U_{:.2f}.pt'.format(result_gzsl_soft.best_acc,
                                                                                                      result_gzsl_soft.best_acc_S_T,
                                                                                                      result_gzsl_soft.best_acc_U_T)
                    torch.save({'model': model.state_dict(), 'ae': ae.state_dict(), 'neta': neta.state_dict()}, model_path)
        ...
        if it % opt.save_interval == 0 and it:
            ...
            # model_path = out_dir + '/Iter_model_{:d}.pth'.format(it)
            # torch.save(model, model_path)
            # ae_path = out_dir + '/Iter_ae_{:d}.pth'.format(it)
            # torch.save(ae, ae_path)
            model_path = out_dir + '/Iter_{:d}.pt'.format(it)
            torch.save({'model': model.state_dict(), 'ae': ae.state_dict(), 'neta': neta.state_dict()}, model_path)
